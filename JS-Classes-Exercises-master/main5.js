...
class ShoppingCart {
  ...
  quantity_check() {
    ...
    this.products.length >= 5
      ? (this.total_price -= ten_perc)
      : (this.total_price = this.no_discount_price);
    //    if you buy 3 items of the same type (e.g. 3 pencils) the 4th is free. So, if you buy 4 pencils you only pay 3, if you buy 8 pencils you only pay 6, etc. Change totalPrice so it considers the free items you get. (Notice that if you buy 3 pencils you pay the 3 of them).
    var count = {};
  ...
  replace(productName, replacementProduct) {
    ...
    array.forEach(function (prod, i) {
      //THIS IS THE NEW ARRAY!!! RETURNS THE OLD PRICES + THE NEW!!!
      if ((check = true)) {
